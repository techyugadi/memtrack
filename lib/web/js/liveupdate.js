...
socket.on('init', function (msg) {
	console.log('Received init data +++++ ' + JSON.stringify(msg));
...
socket.on('gc', function(msg) {
	...
	var xval = msg.ts;
        ...
        baseStats.push([xval, yval]);
	console.log(JSON.stringify(baseStats));
...
socket.on('memused', function(msg) {
	...
	var yval3 = yval / 20;
        heapStats.push([xval, yval]);
	baseStats.push([xval, yval2]);
	leaks.push([xval, yval3]);
...
socket.on('update', function (msg) { 
	...
	} else if (!msg.gc && !msg.leak){
		if (heapStats.length > 100) {
			var w = $('#plotdiv').width();
			$('#plotdiv').css("width", w+5);
		}
		var xval = msg.ts;
                var yval = msg.heapUsed / (1024*1024);
		var yval2 = yval / 10;
		var yval3 = yval / 20;
                heapStats.push([xval, yval]);
		baseStats.push([xval, yval2]);
